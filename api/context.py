...
_search_budget_ctx: ContextVar[Optional[SearchBudget]] = ContextVar("search_budget", default=None)


def set_search_budget(limit: int = 5):
    return _search_budget_ctx.set(SearchBudget(limit))


def reset_search_budget(token):
    _search_budget_ctx.reset(token)


def consume_search_budget() -> bool:
    budget = _search_budget_ctx.get()
    return budget.consume() if budget is not None else True
