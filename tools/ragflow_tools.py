...
# 导入依赖
from rawflow.client import CompatibleRAGFlow as RAGFlow
...
def get_assistant_list() -> str:
    ...
    try:
        ...
        for chat in chat_list:
            dataset_names = []
            # 新版服务(0.27+)直接给出 kb_names，旧版则要从 datasets 里逐个取名字
            kb_names = getattr(chat, 'kb_names', None)
            if kb_names and isinstance(kb_names, list):
                dataset_names = [str(name) for name in kb_names if name]
            else:
                dataset_list = getattr(chat, 'datasets', []) #兼容不同服务版本
                if dataset_list and isinstance(dataset_list,list):
                    # 知识库的name
                    for dataset in dataset_list:
                        # print(dataset)
                        # 旧版是字典，新版可能直接是知识库id字符串，两种都要能取到
                        if isinstance(dataset, dict):
                            dataset_names.append(dataset.get('name', ''))
                        elif not isinstance(dataset, str):
                            dataset_names.append(getattr(dataset, 'name', ''))

            # 拼接下当前助手的信息 + 知识库信息
            # 法律资源小助手  xxxxxx  关联知识库：xx、xxx、xxx
            count_chat_info += f"助手名称:{chat.name};功能介绍：{getattr(chat, 'description', '')}; 关联的知识库：{'、'.join(dataset_names)} \n"
...
def create_ask_delete(chat_name,question)->str:
    ...
    use_chat = None
    try:
        chats = [chat for chat in ragflow_client.list_chats(name=chat_name) if chat.name == chat_name]
        if len(chats) != 1:
            return "提问失败：助手不存在或存在同名助手，请使用唯一的助手名称。"
        ...
        for part in response:
            # 数据存在对象中content上！！
            # print(part.content)
            # 流的最后一个分片是空的结束标记，直接赋值会把答案冲掉，只保留最后一个非空分片
            content = getattr(part, "content", None)
            if content:
                result = content
        # 5. 关闭提问的会话
        # chat -> 关闭 -》  session
        # 临时会话统一在 finally 中清理，覆盖问答异常场景。
        # 6. 返回结果
        return result
    except Exception as e:
        ...
        if session is not None and use_chat is not None:
            try:
                ...
                monitor._emit("error", "RAGFlow临时会话清理失败，请检查服务状态。")
