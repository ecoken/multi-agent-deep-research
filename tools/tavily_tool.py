...
from api.context import consume_search_budget
...
def internet_search(
    ...
    :return: 
    """
    if not consume_search_budget():
        return {"results": [], "error": "本次主任务的网络检索预算已用完（最多5次，所有子任务共享）。请使用已有资料回答，不要重试。"}
    ...
    return tavily_client.search(query = query, topic =  topic,
                                ...
                                include_raw_content = include_raw_content, timeout=20)
